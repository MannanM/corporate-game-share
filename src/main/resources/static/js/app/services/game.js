...
.factory('GameService', ['$http', '$rootScope', function ($http, $rootScope) {
    var service = {};
    ...
    service.Search = function (query, consoleId) {
        console.log('Attempting to search for console: ' + consoleId + ' game named: ' + query);
        //todo: use console id instead of short name, e.g. /v1/consoles/1/games?name
        return $http.get('/v1/games?pageSize=5&console=' + consoleId + '&name=' + query).then(function(response) {
